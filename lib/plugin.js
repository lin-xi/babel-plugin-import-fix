...
var Plugin = function () {
  function Plugin(libraryName, libraryPath, namePolicy, cssDirectory, camel2DashComponentName, camel2UnderlineComponentName, fileName, types) {
      ...
      if (!this.selectedMethods[methodName]) {
        ...
        var getPath = function getPath(name) {
          ...
          if (_this.namePolicy === 'dash') {
            return _this.libraryPath.replace(reg, camel2Dash(name));
          } else if (_this.namePolicy === 'camel') {
            return _this.libraryPath.replace(reg, name);
          } else if (_this.namePolicy === 'underline') {
            return _this.libraryPath.replace(reg, camel2Underline(name));
          }
        };
