  ...
  _createClass(Plugin, [{
    ...
    value: function importMethod(methodName, file) {
      ...
      if (!this.selectedMethods[methodName]) {
        ...
        var getCSSPath = function getCSSPath(name) {
          var reg = new RegExp('\\$\{name\}', 'g');
          var finalPath = 'name';
          if (_this.namePolicy === 'dash') {
            finalPath = _this.cssPath.replace(reg, camel2Dash(name));
          } else if (_this.namePolicy === 'camel') {
            finalPath = _this.cssPath.replace(reg, name);
          } else if (_this.namePolicy === 'underline') {
            finalPath = _this.cssPath.replace(reg, camel2Underline(name));
          }
